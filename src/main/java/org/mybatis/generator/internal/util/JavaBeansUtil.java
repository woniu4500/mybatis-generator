...
public class JavaBeansUtil {
    ...
    public static Method getJavaBeansGetter(IntrospectedColumn introspectedColumn,
        ...
        if (isNewHashMapEnabled(context) && introspectedColumn.isMapColumn()) {
			StringBuilder sb = new StringBuilder();
			sb.append("if (");
			sb.append(property);
			sb.append(" == null ) ");
			sb.append(property);
			sb.append(" = new java.util.HashMap(); "); //$NON-NLS-1$
	...
	private static boolean isNewHashMapEnabled(Context context) {
		Properties properties = context.getJavaModelGeneratorConfiguration().getProperties();
		boolean rc = isTrue(properties.getProperty(PropertyRegistry.MODEL_GENERATOR_NEW_HASHMAP));
		return rc;
	}
